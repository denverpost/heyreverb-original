...
<h3>
  <?php _e('More in','spiegelmagazine'); ?>
  <?php echo $prinz_c->name; ?></h3>
<ul class="more">
  <?php	
...
class PRiNZ_Newsletter extends WP_Widget {
	...
	function PRiNZ_Newsletter() {
        $widget_ops = array('classname' => 'widget_PRiNZ_newsletter', 'description' => __('Newsletter form for Feedburner eMail Subscription','spiegelmagazine') );
		$this->WP_Widget('PRiNZ_newsletter', __('PRiNZ Newsletter','spiegelmagazine'), $widget_ops);
    ...
    function widget($args, $instance) {        
        ...
        $text	= empty($instance['text']) ? __('Signup to our Newsletter','spiegelmagazine') : $instance['text'];
...
<div id="newsletter">
  <form action="http://feedburner.google.com/fb/a/mailverify" method="post" target="popupwindow" onsubmit="window.open('http://feedburner.google.com/fb/a/mailverify?uri=<?php echo $user; ?>', 'popupwindow', 'scrollbars=yes,width=550,height=520');return true" class="clearfix">
    ...
    <input name="submit" class="newsletter-button" type="submit" id="newsleter-submit" tabindex="5" value="<?php _e('Submit','spiegelmagazine'); ?>" />
  </form>
</div>
<?php echo $after_widget; ?>
<?php
...
class PRiNZ_Twitter extends WP_Widget {
	...
	function PRiNZ_Twitter() {
        $widget_ops = array('classname' => 'widget_PRiNZ_twitter', 'description' => __('Display latest Tweets','spiegelmagazine') );
		$this->WP_Widget('PRiNZ_twitter', __('PRiNZ Twitter','spiegelmagazine'), $widget_ops);
    ...
    function widget($args, $instance) {        
        ...
        $title	= empty($instance['title']) ? __('Latest Tweets','spiegelmagazine') : $instance['title'];
        $user	= empty($instance['user']) ? get_option('prinz_twitter') : $instance['user'];
        $link	= $instance['twitter_link'] ? '1' : '0';
        $label	= empty($instance['twitter_label']) ? __('More updates on Twitter','spiegelmagazine') : $instance['twitter_label'];
...
<p>
  <label for="<?php echo $this->get_field_id('twitter_nr'); ?>">
    <?php _e('Number of tweets to show','spiegelmagazine'); ?>
    :</label>
...
<p>
  ...
  <label for="<?php echo $this->get_field_id('twitter_link'); ?>">
    <?php _e('Show link to Twitter','spiegelmagazine'); ?>
  </label>
</p>
<p>
  <label for="<?php echo $this->get_field_id('twitter_label'); ?>">
    <?php _e('Link label','spiegelmagazine'); ?>
    :
...
class PRiNZ_Recent extends WP_Widget {
	...
	function PRiNZ_Recent() {
		...
		$widget_ops = array('classname' => 'widget_prinz_recent', 'description' => __('Display recent posts','spiegelmagazine') );
		$this->WP_Widget('prinz_recent', __('PRiNZ Recent Posts','spiegelmagazine'), $widget_ops);
	...
	function widget($args, $instance) {
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Recent Posts','spiegelmagazine') : $instance['title']);
...
class PRiNZ_Comments extends WP_Widget {
	...
	function PRiNZ_Comments() {
        $widget_ops = array('classname' => 'widget_PRiNZ_comments', 'description' => __('Display recent comments','spiegelmagazine') );
		$this->WP_Widget('PRiNZ_comments', __('PRiNZ Comments','spiegelmagazine'), $widget_ops);
    ...
    function widget($args, $instance) {        
        ...
        $title	= empty($instance['title']) ? __('Recent Comments','spiegelmagazine') : apply_filters('widget_title', $instance['title']);
...
<p>
  <label for="<?php echo $this->get_field_id('comments_nr'); ?>">
    <?php _e('Number of comments','spiegelmagazine'); ?>
    :</label>
...
<p>
  <label for="<?php echo $this->get_field_id('comments_exc'); ?>">
    <?php _e('Length of comment excerpt','spiegelmagazine'); ?>
    :</label>
...
function PRiNZ_recent_comments($rc_count=5, $rc_length=75, $rc_pre='<ul class="recent-comments">', $rc_post='</ul>') {
...
foreach ($comments as $comment) {
	$rc_dots = "...";
	$output .= "\n\t<li><p>" . $comment->comment_author . " ".__('on','spiegelmagazine')." <a href=\"" . get_permalink($comment->ID) . "#comment-" . $comment->comment_ID  . "\" title=\"" . $comment->post_title . "\">" . $comment->post_title . "</a>:<br />" . strip_tags($comment->com_excerpt) . $rc_dots . "</p></li>";
}
$output .= $rc_post;
...
class PRiNZ_About extends WP_Widget {
	...
	function PRiNZ_About() {
        $widget_ops = array('classname' => 'widget_PRiNZ_about', 'description' => __('Display an about section in the Sidebar','spiegelmagazine') );
		$this->WP_Widget('PRiNZ_about', __('PRiNZ About','spiegelmagazine'), $widget_ops);
    ...
    function widget($args, $instance) {        
        ...
        $title	= empty($instance['title']) ? __('About this Site','spiegelmagazine') : $instance['title'];
        $avatar	= $instance['about_avatar'] ? '1' : '0';
        $text	= empty($instance['about_text']) ? __('Your text about you.','spiegelmagazine') : $instance['about_text'];
        $link 	= (int) $instance['about_link'];
        $label	= empty($instance['about_label']) ? __('More about us','spiegelmagazine') : $instance['about_label'];
...
<p>
  ...
  <label for="<?php echo $this->get_field_id('about_avatar'); ?>">
    <?php _e('Show admin\'s avatar','spiegelmagazine'); ?>
  </label>
</p>
<p>
  <label for="<?php echo $this->get_field_id('about_text'); ?>">
    <?php _e('Text','spiegelmagazine'); ?>
    :
...
<p>
  <label for="<?php echo $this->get_field_id('about_link'); ?>">
    <?php _e('Post/page ID for link','spiegelmagazine'); ?>
    :</label>
...
class PRiNZ_Contact extends WP_Widget {
	...
	function PRiNZ_Contact() {
        $widget_ops = array('classname' => 'widget_PRiNZ_contact', 'description' => __('Display contact info in the Sidebar','spiegelmagazine') );
		$this->WP_Widget('PRiNZ_contact', __('PRiNZ Contact Info','spiegelmagazine'), $widget_ops);
    ...
    function widget($args, $instance) {        
        ...
        $title	= empty($instance['title']) ? __('Contact Info','spiegelmagazine') : $instance['title'];
...
<ul style="margin:0">
  ...
  <?php if($email) echo '<li><a href="mailto:'.antispambot($email).'">'.__('Send us an Email','spiegelmagazine').'</a></li>'; ?>
</ul>
<?php echo $after_widget; ?>
<?php
    }
...
<p>
  <label for="<?php echo $this->get_field_id('contact_city'); ?>">
    <?php _e('ZIP City','spiegelmagazine'); ?>
    :
...
<p>
  <label for="<?php echo $this->get_field_id('contact_email'); ?>">
    <?php _e('Email (spam-protected)','spiegelmagazine'); ?>
    :
