...
Description: This plugin creates an image slideshow from the images you upload using the jQuery Cycle plugin. 
You can upload/delete images via the administration panel, and display the images in your theme by using the <code>wp_cycle();</code> template tag, 
which will generate all the necessary HTML for outputting the rotating images.
Version: 0.1.8  - Customized for and integrated into the SpiegelMagazine Theme by Michael Oeser
Author: Nathan Rice
...
function add_wp_cycle_menu() {
	add_submenu_page('themes.php', 'WP-Cycle Settings', 'WP-Cycle', 8, 'wp-cycle', 'wp_cycle_admin_page');
}
...
function wp_cycle_images_admin() { ?>
	...
	<h2><?php _e('WP-Cycle Plugin customized for PRiNZ SpiegelMagazine', 'wp_cycle'); ?></h2>
	<p>Original Plugin by Nathan Rice. You can find the latest version of the Plugin <a href="http://www.nathanrice.net/plugins/wp-cycle/">here.</a></p>
	<h3><?php _e('WP-Cycle Images', 'wp_cycle'); ?></h3>
	
	<table class="form-table">
		<tr valign="top"><th scope="row"><strong>Upload New Image</strong><br /><small>(make sure it has the right dimensions of 940x350 pixels)</small></th>
			<td>
...
function wp_cycle_settings_admin() { ?>
	...
	<h3><?php _e('WP-Cycle Settings', 'wp-cycle'); ?></h3>
	<form method="post" action="options.php">
...
function wp_cycle_scripts() {
	if(!is_admin())
	wp_enqueue_script('cycle', $src = WP_THEME_URL.'/tools/wp-cycle/jquery.cycle.all.min.js', $deps = array('jquery'));
}
...
jQuery(document).ready(function($) {
	$j("#<?php echo $wp_cycle_settings['div']; ?>").cycle({ 
